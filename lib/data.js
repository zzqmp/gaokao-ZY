...
const USE_DB = !!DB_URL;
let _dbHasData = null; // 缓存：DB 是否有数据
...
async function getPool() {
  ...
  // 用 createRequire 运行时加载 pg，防止 Vercel webpack 静态追踪
...
// ====== DB 健康检查（DB 有表但数据为空时降级回 JSON） ======

async function dbHasData() {
  if (_dbHasData !== null) return _dbHasData;
  try {
    const { rows } = await pgQuery("SELECT COUNT(*) as cnt FROM score_rank");
