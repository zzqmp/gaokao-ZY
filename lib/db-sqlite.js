...
function initSchema() {
  ...
  `)
  _db.exec('CREATE INDEX IF NOT EXISTS idx_pending_openid ON wechat_pending_replies(open_id)')
  _db.exec('CREATE INDEX IF NOT EXISTS idx_pending_msgid ON wechat_pending_replies(msg_id)')
}
...
export function savePendingReply(msgId, openId, queryText) {
  const db = getDb()
  db.prepare(`
    INSERT OR REPLACE INTO wechat_pending_replies (msg_id, open_id, query_text, status, updated_at)
    VALUES (?, ?, ?, 'pending', datetime('now', 'localtime'))
  `).run(msgId, openId, queryText)
}

export function updatePendingReply(msgId, replyText) {
  const db = getDb()
  db.prepare(`
    UPDATE wechat_pending_replies SET status = 'done', reply_text = ?, updated_at = datetime('now', 'localtime')
    WHERE msg_id = ?
  `).run(replyText, msgId)
}

/** 获取用户最近一条未送达的待发送回复 */
export function getUserPendingReply(openId) {
  const db = getDb()
  return db.prepare(`
    SELECT * FROM wechat_pending_replies
    WHERE open_id = ? AND status = 'done'
    ORDER BY created_at DESC LIMIT 1
  `).get(openId) || null
}

/** 按 msg_id 获取 pending 记录（用于微信重试路径） */
export function getPendingReplyByMsgId(msgId) {
  const db = getDb()
  return db.prepare(`
    SELECT * FROM wechat_pending_replies WHERE msg_id = ?
  `).get(msgId) || null
}

export function deletePendingReply(msgId) {
  const db = getDb()
  db.prepare('DELETE FROM wechat_pending_replies WHERE msg_id = ?').run(msgId)
}

/** 清理超过 24 小时的过期记录 */
export function cleanExpiredPendingReplies() {
  const db = getDb()
  db.prepare(`
    DELETE FROM wechat_pending_replies
