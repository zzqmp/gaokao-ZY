 ...
 */
import { queryAndReply, getAvailableYears } from '@/lib/gaokao-query';
 ...
 * 微信重试或用户下条消息送达
 *
 * 注意：此处不再调 queryAndReply（handleMessage 已查过），
 * 直接用用户原始文本发 DeepSeek，节省 1-3 秒数据查询时间。
 * Vercel Hobby 计划函数超时 10s，DeepSeek 本身需 ~10s，省功耗。
 */
export async function runAiAnalysisAndStore(text, openId, msgId) {
  try {
    if (!text || !/\d{3}\s*分/.test(text)) return

    // 先确保 DB 中有 pending 记录（供微信重试轮询用）
    await savePendingReply(msgId, openId, text)

    const apiKey = process.env.DEEPSEEK_API_KEY
    if (!apiKey || apiKey === 'sk-your-key-here') {
      await updatePendingReply(msgId, '⚠️ AI 分析未配置，请联系管理员设置 API Key。')
      return
    }

    // 直接用用户问题当 prompt，DeepSeek 本身懂高考志愿
    const resp = await fetch(
      process.env.DEEPSEEK_API_URL || 'https://api.deepseek.com/v1/chat/completions',
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: '你是一个高考志愿填报助手，帮助考生分析分数和位次。回复要简洁、数据驱动，给出具体的填报建议。数据均为历史年份，禁止承诺录取。' },
            { role: 'user', content: `用户查询：${text}\n\n请基于高考数据帮用户分析：1）全省位次范围 2）可报考的大学层次 3）填报建议。` },
          ],
          temperature: 0.3,
          max_tokens: 500,
        }),
    ...
    if (!resp.ok) {
      console.error('[wechat AI] DeepSeek API error:', resp.status, resp.statusText)
      await updatePendingReply(msgId, '⚠️ AI 分析服务暂时不可用（' + resp.status + '），请稍后再试。')
      return
    }
    ...
    if (!content) {
      await updatePendingReply(msgId, '⚠️ AI 分析返回为空，请稍后再试。')
      return
    }
