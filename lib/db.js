...
// 微信待发送回复（AI 分析结果暂存）
export async function savePendingReply(msgId, openId, queryText)    { return (await getMod()).savePendingReply(msgId, openId, queryText) }
export async function updatePendingReply(msgId, replyText)          { return (await getMod()).updatePendingReply(msgId, replyText) }
export async function getUserPendingReply(openId)                   { return (await getMod()).getUserPendingReply(openId) }
export async function getPendingReplyByMsgId(msgId)                 { return (await getMod()).getPendingReplyByMsgId(msgId) }
export async function deletePendingReply(msgId)                     { return (await getMod()).deletePendingReply(msgId) }
export async function cleanExpiredPendingReplies()                  { return (await getMod()).cleanExpiredPendingReplies() }
